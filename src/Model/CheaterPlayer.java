...
public class CheaterPlayer extends Player {
	...
	/*
	 * cheater attack behaviosr choosing the best country's army and and find the
	...
	public ArrayList<Country> cheaterAttackPlayer(ArrayList<Country> countryList) {
		...
		ArrayList<Country> adjacentCountries = ConquestController.getInstance()
				.getAdjacentCountries(maxArmyCountry.getAdjacentCountriesID(), countryList);
		...
		if (enemyArmies > 1 && attackerArmies > 1) {
			...
			AttackResponse attackResponse = ConquestController.getInstance()
					.attackCalculation(maxArmyCountry.getArmy() - 1, enemyArmies);
	...
	/*
	 * cheater move behaviosr choosing the best country's army and find the best
	...
	public void cheaterMovePlayer(ArrayList<Country> countryList) {
		...
		// finding the best country wich has more adjaecent Country from player
		// countries
		...
		CountryHelper countryHelper = new CountryHelper();
		ArrayList<Country> adjacentCountries = ConquestController.getInstance()
				.getAdjacentCountries(selectedCountry.getAdjacentCountriesID(), countryList);
